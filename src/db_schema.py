...
from src.db_sqlite import connect_sqlite, table_columns, table_exists
...
def ensure_schema(
    ...
    *,
    ...
    Returns:
        list of applied changes, empty if already up-to-date.
    """
    ...
    for attempt in range(max(1, retries)):
        try:
            conn = connect_sqlite(path, timeout=5)
            try:
                cur = conn.cursor()
                ...
                missing_required = sorted(_ITEMS_REQUIRED_COLS - cols)
                if missing_required:
                    raise RuntimeError(
                        f"SchemaInvalid: items missing required columns: {missing_required}"
                    )
                ...
                cur.execute(
                    "CREATE INDEX IF NOT EXISTS idx_items_ingest_status ON items(ingest_status)"
                )
                cur.execute("CREATE INDEX IF NOT EXISTS idx_items_context ON items(context)")
                cur.execute(
                    """
                    CREATE UNIQUE INDEX IF NOT EXISTS uq_items_user_fingerprint
                    ON items(user_id, source_fingerprint)
                    """
                )
                ...
                cur.execute(
                    """
                    CREATE INDEX IF NOT EXISTS idx_runs_component_started
                    ON runs(component, started_at)
                    """
                )
                cur.execute(
                    """
                ...
                if record_baseline_if_needed(
                    conn,
                    version=SCHEMA_BASELINE_VERSION,
                    changes=changes,
                ):
                    changes.append(
                        f"record_migration:schema.{SCHEMA_BASELINE_VERSION}"
                    )
            ...
            logger.exception(
                ...
                extra={"request_id": "-", "event": "db.schema.failed"},
            )
            raise
